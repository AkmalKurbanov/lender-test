  ...
  return src([
    ...
    .pipe(
      pug({
        ...
        pretty: true,
        locals: dataFromFile || {},
        // Фильтр для удаления комментариев
        filters: {
...
// обработка файла sprite.html 
function spriteToSvg() {
  return src("app/sprite.html") // Выбираем sprite.html
    .pipe(handleErrors("spriteToSvg"))
    .pipe(rename("sprite.svg")) // Меняем формат на .svg
    .pipe(dest("app")) // Сохраняем в папку app
...
function startwatch() {
  ...
  watch("app/sprite.html", { usePolling: true }, spriteToSvg);
